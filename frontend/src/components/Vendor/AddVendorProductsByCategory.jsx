...
const AddVendorProductsByCategory = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8080/vendor/category/getall`)
  ...
  useEffect(() => {
    if (selectedCid) {
      axios.get(`http://localhost:8080/vendor/brands/getbycid/${selectedCid}`)
  ...
  useEffect(() => {
    if (selectedBid) {
      axios.get(`http://localhost:8080/vendor/product/getallbybid/${selectedBid}`)
  ...
  const fetchAddedProducts = async () => {
    try {
      const [vendorRes, rentalRes] = await Promise.all([
        axios.get(`http://localhost:8080/vendor/productvendor/getbyvid/${vid}`),
        axios.get(`http://localhost:8080/vendor/productrental/getbyvid/${vid}`)
      ]);
  ...
  const handleAddProduct = async (prodid) => {
    try {
      ...
      if (parseInt(selectedCid) === 6) {
        ...
        endpoint = `http://localhost:8080/vendor/productrental/add`;
      } else {
        payload = { prodid, vid, price };
        endpoint = `http://localhost:8080/vendor/productvendor/add`;
      }
